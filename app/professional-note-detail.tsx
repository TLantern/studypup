...
const SPEAKER_COLORS = ['#7FA8FF', '#5DC872', '#F5A623', '#E879A3', '#9B6DFF', '#FF6B6B'];

type SpeakerSegment = { speakerId: number; label: string; color: string; text: string };

function parseSpeakerSegments(transcript: string): SpeakerSegment[] {
  ...
  for (const raw of lines) {
    const line = raw.trim();
    if (!line) continue;
    const match = line.match(/^(Speaker\s+\d+|S\d+|SPEAKER\s+\d+)\s*:\s*(.*)/i);
    if (match) {
      if (current) segments.push(current);
      const key = match[1].toLowerCase().replace(/\s+/, '');
      if (!speakerMap.has(key)) speakerMap.set(key, speakerMap.size + 1);
      const id = speakerMap.get(key)!;
      current = { speakerId: id, label: `Speaker ${id}`, color: SPEAKER_COLORS[(id - 1) % SPEAKER_COLORS.length], text: match[2] };
    } else if (current) {
      current.text = current.text ? `${current.text} ${line}` : line;
    } else {
      current = { speakerId: 1, label: 'Speaker 1', color: SPEAKER_COLORS[0], text: line };
    }
  }
  if (current) segments.push(current);
  return segments;
...
function SpeakerBubble({ segment }: { segment: SpeakerSegment }) {
  return (
    <View style={speakerStyles.row}>
      <View style={[speakerStyles.avatar, { backgroundColor: segment.color }]}>
        <Text style={speakerStyles.avatarText}>{segment.speakerId}</Text>
      </View>
      <View style={speakerStyles.content}>
        <Text style={speakerStyles.label}>{segment.label}</Text>
        <Text style={speakerStyles.text}>{segment.text}</Text>
      </View>
    </View>
  );
}
...
const speakerStyles = StyleSheet.create({
  row: { flexDirection: 'row', gap: scaleSize(10), alignItems: 'flex-start', marginBottom: scaleSize(16) },
  avatar: { width: scaleSize(30), height: scaleSize(30), borderRadius: scaleSize(15), alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  avatarText: { fontFamily: Platform.select({ ios: 'System', android: 'sans-serif', default: 'System' }), fontSize: scaleFont(13), fontWeight: '700', color: '#fff' },
  content: { flex: 1 },
  label: { fontFamily: Platform.select({ ios: 'System', android: 'sans-serif', default: 'System' }), fontSize: scaleFont(12), fontWeight: '700', color: '#0D0D0F', marginBottom: scaleSize(2) },
  text: { fontFamily: Platform.select({ ios: 'System', android: 'sans-serif', default: 'System' }), fontSize: scaleFont(14), color: '#444', lineHeight: scaleFont(20) },
});
...
export default function ProfessionalNoteDetailScreen() {
  ...
  const metaDateStr = (() => {
    ...
    if (!audioDurationMs) return dateStr;
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <View style={styles.headerRow}>
        ...
        <View style={{ width: scaleSize(22), height: scaleSize(22) }} />
        {note ? (
          <Pressable
            hitSlop={12}
      ...
      >
        ...
        <Text style={styles.titleSecond} numberOfLines={3}>{subtitle}</Text>
        ...
        <View style={styles.metaRow}>
          ...
          {metaDateStr ? <Text style={styles.metaDate}>{metaDateStr}</Text> : null}
        </View>
                  ...
                  newSound.setOnPlaybackStatusUpdate((status) => {
                    if (status.isLoaded) {
                      ...
                      if (status.durationMillis && status.durationMillis > 0) {
                        ...
                        setAudioDurationMs((prev) => prev ?? status.durationMillis ?? null);
                      }
      ...
      <Modal visible={transcriptOpen} transparent animationType="slide" onRequestClose={() => setTranscriptOpen(false)}>
        <View style={[styles.transcriptRoot, { paddingTop: insets.top }]}>
          ...
          >
            {parseSpeakerSegments(note?.transcript ?? '').map((seg, i) => (
