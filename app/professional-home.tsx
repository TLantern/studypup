...
export default function ProfessionalHomeScreen() {
  ...
  // Toggle: set EXPO_PUBLIC_VIRAL_MODE=true in .env.local to enable the viral layout
  if (process.env.EXPO_PUBLIC_VIRAL_MODE === 'true') {
    router.replace({ pathname: '/viral-professional-home', params: openFolders ? { openFolders } : {} });
    return null;
  }
  ...
  // Pick up a transcript saved by capture-meeting when the user ends a meeting
  useEffect(() => {
    AsyncStorage.getItem('@studypup/pending_meeting_transcript').then(async (raw) => {
      if (!raw) return;
      await AsyncStorage.removeItem('@studypup/pending_meeting_transcript');
      let transcript = '';
      let name = 'Meeting Notes';
      try { ({ transcript, name } = JSON.parse(raw)); } catch {}
      if (!transcript) return;
      setOverlayAlmostDone(false);
      setShowProcessingOverlay(true);
      try {
        const id = await generateNoteFromTranscript(transcript, { title: name });
        setOverlayAlmostDone(false);
        setShowProcessingOverlay(false);
        router.push({ pathname: '/professional-note-detail', params: { generated: '1', noteId: id } });
      } catch (e: any) {
        setShowProcessingOverlay(false);
        Alert.alert('Error', 'Could not generate notes from the meeting transcript.');
      }
    });
  }, []);
  ...
  useEffect(() => {
    ...
    for (const id of pendingJobIdsRef.current) {
      if (!activeJobIds.has(id) && !navigatedJobsRef.current.has(id)) {
        const note = getAllProNotes().find((n) => n.id === id);
        if (note) {
          ...
          router.push({
            ...
            params: { generated: '1', noteId: note.id, title: note.title, subtitle: note.subtitle },
          });
        }
      }
    }
  ...
  const pickAudio = async () => {
    try {
      ...
      // Route through the durable job pipeline — shows a "Processing…" card and
      // survives the app being backgrounded/killed mid-transcription.
